...
function computepw() {
    //yearsInput.value
    genpw({lifetime: + 246, budget: + 10000000}) //  costInput.value})
        .then(function(state) {
            password = state;
            showPasswords();
        }).catch(function(e) {
...
// load
/* Moved to dstools.js. As we ned to contol if this is caled on page load as we do not need this in pub-pri key approach
...
document.getElementById("text_area").onclick = function() {
    console.log("text_area gets called: ");
    this.select(); 
};
// button
document.getElementById("genBut").onclick = function() {
    console.log("populate_button .onclick = function() gets called: ");
    populateform(8);
};
// range
document.getElementById("outupdate_txt").oninput = function() {
    console.log("outupdate_txt .onclick = function() gets called: ");
    outputUpdate(this.value); };
*/

function initialiseArray() {
// genpass
    //console.log("Initialise Array() gets called: ");
    words = [
...
function generatepass(plength) {
    var arraySize = words.length;
    var randomNum = Math.floor(Math.random() * arraySize);
    //console.log("words length: " + arraySize);
    //console.log("randomNum: " + randomNum);
    var passphrase = words[randomNum];
    var iterSize = plength - 1;
...
function populateform(enterlength) {
    console.log("populateform() gets called: ");
    //document.pgenerate.output.value = generatepass(enterlength)
    currentpassphrase = generatepass(enterlength);
    passphraseOutput.textContent = currentpassphrase;
    // we may hve to set this
    //entropyOutput.textContent = password.entropy.toFixed(2);
...
function copyHash() {
    navigator.clipboard.writeText(hash_value)
        .catch(function(e) {
            console.error('Clipboard copy failed:', e);
            alert('Failed to copy to clipboard.');
        });
...
function outputUpdate(n) {
    document.querySelector('.words').value = n;
}
